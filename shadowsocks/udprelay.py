...
class TCPRelayHandler(object):
        ...
        if self._stage == STAGE_RSP_ID:
            if cmd == CMD_CONNECT:
                ...
                if self._local_id == local_id:
                    ...
                    common.connect_log("TCPonUDP connect %s:%d from %s:%d via port %d" % (remote_addr, remote_port, addr[0], addr[1], self._config['server_port']))
                else:
                    # ileagal request
                    rsp_data = self._pack_rsp_data(CMD_DISCONNECT, RSP_STATE_EMPTY)
                    self._write_to_sock(rsp_data, self._local_sock, addr)
    ...
    def _log_error(self, e):
        logging.error('%s when handling connection from %s via port %d' %
                      (e, self._client_address.keys(), self._config['server_port']))
...
class UDPRelay(object):
    ...
    def _handle_server(self):
        ...
        if not client_pair:
            ...
            common.connect_log("UDP data to %s:%d from %s:%d via port %d by UID %d" %
